...
class TicketLogService:
    ...
    def _fetch_record(cursor, record_id):
        cursor.execute(
            "SELECT id, ticket_id, user_id, start_time, end_time, status, complete_date, note FROM ticket_time_log WHERE id = %s",
            (record_id,)
        )
        row = cursor.fetchone()
        if row:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if action in ["start", "resume"]:
                    ...
                    cursor.execute(
                        "INSERT INTO ticket_time_log (ticket_id, user_id, start_time, status) VALUES (%s, %s, %s, %s)",
                    ...
                    cursor.execute(
                        "SELECT id FROM ticket_time_log WHERE ticket_id = %s AND user_id = %s AND status = 1 ORDER BY id DESC LIMIT 1",
                        (ticket_id, user_id)
                    )
                    ...
                    # Update active log
                    cursor.execute(
                        "UPDATE ticket_time_log SET end_time = %s, status = 2, note = %s WHERE id = %s",
                    ...
                    cursor.execute(
                        "SELECT id, status FROM ticket_time_log WHERE ticket_id = %s AND user_id = %s AND status IN (1, 2)",
                        (ticket_id, user_id)
                    )
                    ...
                    cursor.execute(
                        "UPDATE ticket_time_log SET end_time = %s, status = 0, complete_date = %s, note = %s WHERE ticket_id = %s AND user_id = %s AND status = 1",
                        (now_dt, now_dt, note, ticket_id, user_id)
                    )
                    # Update status=2 (paused) logs to complete
                    cursor.execute(
                        "UPDATE ticket_time_log SET status = 0, complete_date = %s, note = %s WHERE ticket_id = %s AND user_id = %s AND status = 2",
    ...
    def get_active_logs(ticket_id: int, user_id: int):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT id, ticket_id, user_id, start_time, end_time, status, complete_date, note FROM ticket_time_log WHERE ticket_id = %s AND user_id = %s AND status IN (1, 2) ORDER BY id ASC",
                    (ticket_id, user_id)
                )
    ...
    def get_ticket_log_history(ticket_id: int, user_id: int):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT id, ticket_id, user_id, start_time, end_time, status, complete_date, note FROM ticket_time_log WHERE status != 1 AND ticket_id = %s AND user_id = %s ORDER BY id DESC",
                    (ticket_id, user_id)
                )
                results = cursor.fetchall()
                for row in results:
    ...
    def create_log(payload):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO ticket_time_log (ticket_id, user_id, start_time, end_time, status, complete_date, note) VALUES (%s, %s, %s, %s, %s, %s, %s)",
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if update_fields:
                    ...
                    query = f"UPDATE ticket_time_log SET {', '.join(update_fields)} WHERE id = %s"
    ...
    def delete_log(log_id: int):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if not record:
                    raise HTTPException(status_code=404, detail="Ticket log not found")
                cursor.execute("DELETE FROM ticket_time_log WHERE id = %s", (log_id,))
                conn.commit()
                return True
        except HTTPException:
            raise
    ...
    def check_current_work_status(user_id: int):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT ticket_id FROM ticket_time_log WHERE user_id = %s AND status IN (1) ORDER BY id DESC LIMIT 1",
                    (user_id,)
                )
